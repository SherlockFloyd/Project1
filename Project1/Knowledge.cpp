...
void test21()
{
    // 存储在数组中，栈区
    char str1[] = "hello world!";
    cout << *str1 << endl; // str1是一个char类型的指针，直接对其解引用会输出该类型对应的长度，即仅输出h
    *str1 = 'H';           // 仅会修改第一个字符
    cout << str1 << endl;  // 使用字符数组名进行输入输出，会自动输出整个字符串，直到遇到结束符‘\0’结束

    // str1 = "hello kitty!";  // 错误，字符数组名是常量，不能修改
    strcpy(str1, "hello kitty"); // 正确，使用字符串复制函数进行修改字符数组内容
    // cin >> str1;                 // 正确，数组内容可以修改，可以使用数组名整体输入输出

    char *pstr1 = str1; // 使用指针指向字符串数组可以进行与字符串数组相同的操作
    ...
    strcpy(str2, "hello kitty"); // 正确，使用字符串复制函数进行修改字符数组内容
    // cin >> str2;                 // 正确，数组内容可以修改，可以使用数组名整体输入输出

    // 存储在文字常量区
    char *str3 = "hello world!";
    str3 = "hello kitty!"; // 正确，将该指针指向一个新的常量区
    // strcpy(str3, "hello dog!"); // 错误，该指针指向常量区，不能通过指针修改文字常量区的值

    /*使用指针在定义时初始化或使用const修饰的字符串无法进行修改，只读*/
}

// 数组指针
/*数组指针本质上是一个指针，指向一个数组，+1跳向下一个数组，定义：类型 (*指针变量)[数组元素个数] ，小括号用来指定运算顺序*/
/*
int (*p)[N]  // 数组指针，本质上是一个指针，指向一个数组，+1跳向下一个数组，
int *p[N]  // 指针数组，是一个数组，数组元素为指针类型
int **p  // 指针的指针
*/
void test22()
{
    int nums2[4][5];
    int(*pnums2)[5]; // pnums2是一个一维数组指针，指向一个一维数组，一维数组的长度为5个int长度
    pnums2 = nums2;
    /* &nums2[0][0] = nums2 = pnums2 */
    cout << "地址&nums2[0][0]=" << &nums2[0][0] << endl;
    cout << "地址&nums2[0][0]+1=" << &nums2[0][0] + 1 << endl; // 与上一行的差值为4B，即一个int型长度
    cout << "地址nums2=" << nums2 << endl;
    cout << "地址nums2+1=" << nums2 + 1 << endl; // 与上一行地址的差值为20B，即一行int型长度，nums是一个一维数组指针，+1则指向下一行首地址,
    cout << "地址pnums2=" << pnums2 << endl;
    cout << "地址pnums2+1=" << pnums2 + 1 << endl; // 一维数组指针，+1则指向下一行首地址
    /*
    一维数组指针配合二维数组用，要保证列值相同
    数组指针常用于函数调用，传递二维数组，形参使用数组指针的形式表示

    拓展
    int (*p)nums[4][5]  // 二维数组指针，指针指向一个二维数组
    int (*p)nums[4][5][6]  // 三维数组指针
    ...
    */
    ...
    int *pnums4, i = 6;
    pnums4 = nums4;
    cout << "nums4[i]=" << nums4[i] << endl;
    cout << "pnums4[i]=" << pnums4[i] << endl;
    cout << "*(nums4 + i)=" << *(nums4 + i) << endl;
    cout << "*(pnums4 + i)=" << *(pnums4 + i) << endl;

    // pnums4 = &nums4[3];  // 正确
    // nums4 = &nums4[3];  // 报错
    cout << "nums4的地址=" << &nums4 << endl;
    cout << "pnums4的地址=" << &pnums4 << endl; // 这两行结果不同
    // 相同点
    /*
    读取元素的用法相同
    nums4[i] = pnums4[i] = *(nums4 + i) = *(pnums4 + i)
    */

    // 不同点
    /*
    1、数组名是常量，指针是变量
        可以使用 = 给指针变量赋值，表示将该指针指向新的内存空间，而不能使用 = 给数组名赋值
    2、取地址结果不同
        对数组名取地址获得的是数组的首地址，结果为数组指针
        对指针变量取地址获得的是指针的地址，结果为指针的指针
    */
    /*----------------------------------------------------------------------------------------------------------*/
    // 数组名取地址&，会变成数组指针
    /*数组名和&数组名输出结果相同，均为数组首地址，只是含义不同*/
    // 一维数组
    int nums1[10];
    int *pnums1 = nums1; // 对pnums1的操作与对nums1的操作相同
    cout << "地址nums1=" << nums1 << endl;
    cout << "地址nums1+1=" << (nums1 + 1) << endl; // 与上一行地址值相差4B，即一个int型长度，nums1数组名为第一个元素的地址，+1移动到后一个元素

    // 数组指针，对数组名取地址
    cout << "地址&nums1=" << &nums1 << endl;
    cout << "地址&nums1+1=" << (&nums1 + 1) << endl; // 与上一行地址值相差40B，即一行int型长度，nums1数组名取地址，变成数组指针，该指针的长度为数组长度，+1跳到下一个数组的地址，即向后移动一个数组的长度
    /*
    nums1与&nums1输出的地址值相同，只是含义不同
    */

    // 二维数组
    int nums3[4][5];
    cout << "地址nums3=" << nums3 << endl;
    cout << "地址nums3+1=" << nums3 + 1 << endl; // 与上一行地址值相差20B，即一行int型长度，二维数组名是一个一维数组指针
    cout << "地址&nums3=" << &nums3 << endl;
    cout << "地址&nums3+1=" << &nums3 + 1 << endl; // 与上一行地址值相差80B，即4*5个int型长度，二维数组名取地址是一个二维数组指针

    /*----------------------------------------------------------------------------------------------------------*/
    // 数组指针解引用*，指指针的类型发生变化，降维
    /*
    一维数组指针取*，结果为由原来的一维数组指针变为元素指针，指向一维数组的第0个元素的地址，地址值不变，含义发生变化
    二维数组指针取*，结果为由原来的二维数组指针变为一维数组指针，指向二维数组的第0行的地址，地址值不变，含义发生变化
    ...
    */
    int nums5[4][5];
    int(*pnums5)[5];
    pnums5 = nums5;
    cout << "nums5=" << nums5 << endl;         // 一维数组指针，值为二维数组中第0行的首地址
    cout << "nums5+1=" << nums5 + 1 << endl;   // 与上一行相差20B，值为第1行的行地址
    cout << "*nums5=" << *nums5 << endl;       // 元素指针，值为第0行第0列元素的地址
    cout << "*nums5+1=" << *nums5 + 1 << endl; // 与上一行相差4B，值为第0行第1列元素的地址

    cout << "pnums5=" << pnums5 << endl;         // 一维数组指针，值为二维数组中第0行的首地址
    cout << "pnums5+1=" << pnums5 + 1 << endl;   // 与上一行相差20B，值为第1行的行地址
    cout << "*pnums5=" << *pnums5 << endl;       // 元素指针，值为第0行第0列元素的地址
    cout << "*pnums5+1=" << *pnums5 + 1 << endl; // 与上一行相差4B，值为第0行第1列元素的地址
}
