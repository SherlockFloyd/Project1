...
seekg和seekp都是流（istream和ostream）类的成员函数，用于移动文件指针。它们的区别在于：
seekg用于定位输入流（istream）的文件指针，即读取指针。
seekp用于定位输出流（ostream）的文件指针，即写入指针。
这两个函数的参数和作用是相同的。参数通常是一个streampos类型的值，表示文件指针要移动到的位置。也可以是一个off_type类型的值，表示将文件指针移动多少个字节。
